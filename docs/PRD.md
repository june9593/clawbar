...
| F-27 | 桌面宠物窗口 | P1 | 透明 frameless BrowserWindow，🦞 动画角色，拖拽，点击唤起聊天 |
| F-28 | 宠物 ↔ 聊天集成 | P1 | 状态同步（连接/断开/审批），右键快捷菜单 |
| F-29 | 宠物动画与状态 | P2 | CSS 动画：idle 呼吸、hover 挥手、审批弹跳、断连打瞌睡 |
...
| 桌面宠物窗口 | 独立透明 BrowserWindow | 与主窗口解耦，可独立拖拽和动画，不干扰聊天 UI |

---

## 7. 桌面宠物 🦞（F-27 ~ F-29）

桌面龙虾宠物——类似经典的 Microsoft Clippy 或瑞星小狮子，但角色是 OpenClaw 的 🦞 龙虾。宠物是 ClawBar 的默认待机态，用户不聊天时桌面上常驻一只会动的小龙虾。

### 用户故事

- **US-27**: 作为用户，我希望桌面上有一只龙虾宠物，点击即可唤起 OpenClaw 聊天窗口
  - [ ] 启动时宠物出现在桌面上（always-on-top）
  - [ ] 点击宠物 → 打开/关闭聊天窗口（同 tray 点击逻辑）
  - [ ] 宠物可拖拽到桌面任意位置
  - [ ] 位置跨重启持久化（存入 `~/.clawbar/settings.json`）
- **US-28**: 作为用户，我希望宠物状态反映连接和通知情况
  - [ ] 已连接 → 开心/呼吸动画（idle）
  - [ ] 断开连接 → 打瞌睡动画（sleepy）
  - [ ] 审批通知到达 → 弹跳/抖动动画（bounce）
  - [ ] hover 时挥手/摇爪子
- **US-29**: 作为用户，我希望右键宠物有快捷菜单
  - [ ] 右键菜单项：Show Chat · Settings · Classic/Compact 切换 · Quit
  - [ ] 菜单操作与 tray 右键菜单一致
...
| 项目 | 方案 |
|------|------|
| 窗口 | 独立透明 frameless BrowserWindow，`transparent: true`，`alwaysOnTop: true`，~100×100px |
| 动画 | CSS animations（idle 呼吸、弹跳、摇摆），可升级为 sprite sheet 或 Lottie |
| 交互 | 宠物替代 tray 成为主要交互入口，tray 保留为 fallback |
| 拖拽 | `-webkit-app-region: drag` 或鼠标事件手动实现 |
| 状态 | 监听 `ws:status` 和 `ws:approval` IPC 事件驱动动画切换 |

### 优先级

| ID | 功能 | 优先级 |
|----|------|--------|
| F-27 | 桌面宠物窗口 + 基础交互（显示、拖拽、点击） | P1 |
| F-28 | 宠物 ↔ 聊天窗口集成（状态同步、右键菜单） | P1 |
| F-29 | 宠物动画与状态表现（idle、sleep、bounce） | P2 |
